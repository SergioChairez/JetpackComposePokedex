...
internal fun FetchGenerationVersionResponseDTO.asDomain(): List<VersionGroup> {
    return versionGroups.map { result ->
        VersionGroup(
            name = result.name,
            url = result.url
        )
...
internal fun SearchPokemonResponseDTO.asDomain(): PokemonModel =
    PokemonModel(
        abilities = abilities,
        base_experience = baseExperience,
        forms = forms,
